...
class TestData:
    ...
    region_enum = 0
    region_name = "US915"
    dpkg_output = """Package: helium_gateway\n
    ...
    expected_summary = {
        'region': region_name,
        'key': pubkey_decoded,
        'gateway_version': "1.0.0~alpha.23",
        'validator': {
    ...
    def test_client(self):
        with GatewayClient(f'localhost:{TestData.server_port}') as client:
            ...
            with patch('subprocess.check_output',
                       return_value=TestData.dpkg_output.encode('utf-8')):
                self.assertEqual(client.get_gateway_version(),
                                 TestData.expected_summary['gateway_version'])
                self.assertEqual(client.get_summary(), TestData.expected_summary)
